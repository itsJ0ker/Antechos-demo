...
const ProfessionalsManager = () => {
  ...
  const [formData, setFormData] = useState({ name: '', role: '', image_url: '', short_bio: '', website_url: '', order_index: 0, is_active: true });

  useEffect(() => { fetchData(); }, []);
  const fetchData = async () => { const { data } = await supabase.from('marketplace_professionals').select('*').order('order_index'); setItems(data || []); setLoading(false); };
  const resetForm = () => { setFormData({ name: '', role: '', image_url: '', short_bio: '', website_url: '', order_index: items.length, is_active: true }); setEditingId(null); };
  
  const handleSave = async () => {
    if (!formData.name.trim()) { alert('Name required'); return; }
    setSaving(true);
    try {
      if (editingId) { await supabase.from('marketplace_professionals').update(formData).eq('id', editingId); }
      else { await supabase.from('marketplace_professionals').insert([formData]); }
      alert('Saved!'); resetForm(); fetchData();
    } finally { setSaving(false); }
  };

  const handleDelete = async (id) => { if (!confirm('Delete?')) return; await supabase.from('marketplace_professionals').delete().eq('id', id); fetchData(); };

  if (loading) return <div>Loading...</div>;

  return (
    <div className="space-y-6">
      <h3 className="text-xl font-bold">Featured Professionals</h3>
      <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-6">
        <h4 className="text-lg font-bold mb-4">{editingId ? 'Edit' : 'Add'} Professional</h4>
        <div className="grid gap-4">
          ...
          <textarea placeholder="Short Bio" value={formData.short_bio} onChange={(e) => setFormData({ ...formData, short_bio: e.target.value })} className="px-4 py-2 border-2 rounded-lg" rows={2} />
          <input type="url" placeholder="Website URL" value={formData.website_url} onChange={(e) => setFormData({ ...formData, website_url: e.target.value })} className="px-4 py-2 border-2 rounded-lg" />
