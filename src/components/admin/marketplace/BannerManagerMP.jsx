...
const BannerManagerMP = () => {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [formData, setFormData] = useState({ image_url: '', link_url: '', order_index: 0, is_active: true });

  useEffect(() => { fetchData(); }, []);

  const fetchData = async () => {
    try {
      const { data } = await supabase.from('marketplace_banner').select('*').order('order_index');
      setItems(data || []);
    } catch (error) {
  ...
  const resetForm = () => {
    setFormData({ image_url: '', link_url: '', order_index: items.length, is_active: true });
    setEditingId(null);
  };

  const handleSave = async () => {
    if (!formData.image_url.trim()) { alert('Image URL required'); return; }
    setSaving(true);
    try {
      if (editingId) {
        await supabase.from('marketplace_banner').update(formData).eq('id', editingId);
      } else {
        await supabase.from('marketplace_banner').insert([formData]);
      }
      alert('Saved!');
      resetForm();
      fetchData();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await supabase.from('marketplace_banner').delete().eq('id', id);
      fetchData();
    } catch (error) {
      alert('Error: ' + error.message);
    }
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-xl font-bold">Banner Section</h3>
      <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-6">
        <h4 className="text-lg font-bold mb-4">{editingId ? 'Edit' : 'Add'} Banner</h4>
        <div className="grid gap-4">
          ...
          <input type="url" placeholder="Link URL (optional)" value={formData.link_url} onChange={(e) => setFormData({ ...formData, link_url: e.target.value })} className="px-4 py-2 border-2 rounded-lg" />
        ...
        {items.map((item) => (
          <div key={item.id} className="p-4 flex justify-between items-center hover:bg-gray-50">
            <div className="flex items-center gap-4">
              <img src={item.image_url} alt="Banner" className="h-20 w-40 object-cover rounded" />
...
export default BannerManagerMP;
